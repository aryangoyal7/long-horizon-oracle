...
\author{Aryan Goyal}
\date{July 18, 2026 (updated July 23)}
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{Output MLP.} A two-layer MLP maps the concatenation to four
        ...
        \begin{itemize}[leftmargin=1.5em]
          \item $p_{ol}$: probability that the current moment is open-loop
                \emph{unstable} (a sigmoid; this is the decision output; note
                the convention, high $p_{ol}$ means unstable),
          \item $\hat\lambda_{ol}$: predicted open-loop divergence rate
                (a regression; calibrates the head and is what the runtime
                switch thresholds),
          \item $p_{cl}$, $\hat\lambda_{cl}$: the same pair for the closed-loop
                channel. As run, the deployed head (run 1) trains only the two
                open-loop outputs; the closed-loop pair was trained separately
                as run 5, a diagnostic head on the closed-loop labels
                (Section \ref{sec:clhead}), and no runtime rule uses it.
        \end{itemize}
\end{enumerate}
...
outside the network. The predictor sets it with one comparison, made at every
replan:

\begin{center}
\begin{tabular}{@{}lll@{}}
\toprule
Predicted regime & Executed chunk & Reason \\
\midrule
open-loop stable ($\hat\lambda_{ol} \leq \delta$) & $k = 16$, commit &
  the plant absorbs errors; \\
 & & each replan would inject one \\
open-loop unstable ($\hat\lambda_{ol} > \delta$) & $k = 4$, replan sooner &
  errors grow here, so react \\
 & & more often, but not per step \\
\bottomrule
\end{tabular}
\end{center}

Two points prevent misreading this table. First, stability means commitment:
the more stable the moment, the \emph{longer} the executed chunk. Short chunks
are reserved for unstable moments. Second, the unstable branch uses $k = 4$
and not $k = 1$, because per-step replanning during contact was measured to be
harmful in the Stage 1a rollouts (0.60 to 0.67 success against 0.80 or better
for committed execution) and the closed-loop labels explain why: the policy's
own corrections inject error at 90 to 99 percent of stamps. Reacting more
often than the plant demands is itself a failure mode.

Because every compared execution rule runs the exact same frozen policy
weights, any performance difference is attributable to the switching rule
alone. That attribution is the experiment; baking the signal into policy
training would destroy it. An earlier version of this section described a
planned three-case rule (with a separate closed-loop prediction choosing
between reacting and flagging the segment for data-side repair); the deployed
rule is the two-case rule above, and data-side repair remains future work.
...
Supervised pairs are (cached features, labels) per stamp. The full inventory,
all labeled with the perturb-and-replay oracle at $K{=}24$, $N{=}8$, stride 2,
...
\begin{tabular}{@{}llll@{}}
\toprule
Dataset & Stamps & $\sigma_u$ & Role in training \\
\midrule
lift & 2{,}485 & measured & pool \\
can & 9{,}254 & measured & pool (mostly-stable balance) \\
square & 12{,}723 & measured & pool + primary eval \\
tool\_hang & 45{,}633 & measured & pool + primary eval \\
MimicGen stack\_d0 & 8{,}445 & interim & pool (contact diversity) \\
MimicGen square\_d0 & 12{,}874 & borrowed & pool + transfer check \\
LIBERO-Long (10 files) & $\sim$63k & interim & pool (run 3b onward) \\
policy rollouts (lift, can) & included & measured & pool (runtime states) \\
\bottomrule
\end{tabular}
\end{center}

The preparation this required (labeling the lift and can policy rollouts so the
head sees runtime states and not only the demo tube, regenerating MimicGen
image observations and extracting their features in the MimicGen environment
stack, and adapting the extractor to LIBERO's observation keys) is complete.
The deployed head (run 1) trains on the eight Panda-robot feature sets, 82{,}212
train and 20{,}147 validation stamps; run 3b adds LIBERO for 145{,}378 train
stamps.
...
All five steps have run. Their results are Sections \ref{sec:stage1b} and
\ref{sec:stage1c}.
