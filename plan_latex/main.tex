...
\author{Aryan Goyal}
\date{July 2026 (status and results updated July 22)}
...
%=====================================================================
\section{Status and results as of July 22, 2026}
\label{sec:status}
...
July 21, and the surviving checkpoint grids were then deleted by a fault in
the relaunch script, which auto-answered robomimic's overwrite prompt. The
fresh full-length reruns launched that afternoon therefore carry the whole
pipeline; as of the morning of July 22 they are healthy at epochs 1,425 to
1,643 of 2,000 and finish around midday UTC. The launch script now refuses
to start over an existing experiment directory, and checkpoint selection
backs up each grid to a second disk before evaluating it. Total slip from
both incidents: about one day. Remaining sequence: per-task checkpoint
selection as trainings finish (July 22), the switch-versus-fixed-$k$
rollouts (July 22 to 23), then automatic shutdown. The design, the simulator
version conflict that shapes the infrastructure, both incidents, and the
zero-shot decision for the head are documented in the Stage 2 companion
document.
