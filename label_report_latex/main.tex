...
Before the full detail, three questions that decide how to read everything
below: what exactly is perturbed, what differs between the two channels, and
whether the channels are measured on the same places.
...
OL stable   & commit long $k$ & commit long $k$ & commit long $k$
              (replanning destabilizes) \\
OL deadband & commit long $k$ & commit long $k$
              (fewer replans, fewer errors) & commit long $k$ \\
OL unstable & replan per step
              (feedback rescues) & short $k$ & intermediate $k$
              (minimize injected noise) \\
\bottomrule
\end{tabular}
\end{center}
...
\toprule
Task & Channel & occupancy s/d/u & mean run & median run &
transitions/demo & mass in runs $\geq$3 stamps \\
\midrule
