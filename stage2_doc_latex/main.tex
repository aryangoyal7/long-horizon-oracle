...
percent, what survived, and the restart), and the open decisions for the
evaluation phase.
\end{abstract}
...
%=====================================================================
\section{Status as of July 20, 2026}
\label{sec:status}
...
July 21 scratch-disk event (Section \ref{sec:incident}); their surviving
checkpoint grids end at epochs 1,550 to 1,700, past the quality plateau, so
checkpoint selection runs on those while fresh full-length reruns provide
completion insurance in parallel. The evaluation bridge is built and
smoke-tested, the zero-shot decision for the predictor head is taken, and the
remaining sequence is: selection (evening of July 21), then the fixed-$k$
sweep plus predictor cells (July 22).
