...
CK=$LH_DIR/results/training/dp_square/dp_square_image/20260712190622/models/model_epoch_950_image_v15_success_0.9.pth
OUT=$LH_DIR/results/ksweep/square
run() { # mode k ks ku gpu
  ...
  MUJOCO_GL=egl CUDA_VISIBLE_DEVICES=$gpu $PY $LH_DIR/impl/eval/eval_k_sweep.py \
    ...
    --n-episodes 50 --horizon 400 --seed 0 --out "$OUT/$name.json"
}
queue() { # gpu cells...
  local gpu=$1; shift
